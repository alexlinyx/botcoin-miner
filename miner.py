...
class BotcoinMiner:
    ...
    def solve(self, challenge: Dict, previous_artifact: str = None, failed_constraints: list = None) -> str:
        ...
        # Base prompt
        prompt = f"""You are solving a BOTCOIN mining challenge. This requires precise multi-hop reasoning.
...
You MUST fix these specific issues. Re-read the document carefully.
Common mistakes:
1. Wrong word count - count EXACTLY
2. Wrong company answers - verify each question against the document
3. Wrong calculations - recalculate primes and equations step by step
4. Wrong acrostic - verify first letters of first 8 words match target

Now construct a NEW artifact that fixes these issues."""
        else:
            prompt += """

=== PHASE 1: ANSWER QUESTIONS CAREFULLY ===
For EACH question, you MUST:
1. Find the EXACT company name in the document
2. Verify it matches one of the valid company names above
3. Double-check by searching for contradicting information

Output your answers as:
Q1: [exact company name] - [brief verification from document]
Q2: [exact company name] - [brief verification from document]
... (all 10 questions)

=== PHASE 2: EXTRACT REQUIRED VALUES ===
From your verified answers, extract:
- Required cities/countries/names
- Employee counts and revenue figures
- Any other values needed for constraints

=== PHASE 3: CALCULATE PRECISE VALUES ===
Show ALL work for arithmetic:
- nextPrime(X): test X, X+1, X+2... until prime found
- A+B=C: show each value and sum
- Verify result is prime if required

=== PHASE 4: CONSTRUCT THE ARTIFACT ===
Build artifact satisfying ALL constraints:
- Count words EXACTLY
- Verify all required words are included
- Check no forbidden letters appear
- Verify acrostic spells target

=== OUTPUT FORMAT ===
First show your work, then output ONLY the final artifact on its own line:
ARTIFACT: [your artifact here]"""

        prompt += f"""

Remember: The artifact must be EXACTLY ONE LINE after "ARTIFACT:" - no other text after it."""
        ...
        return artifact, reasoning_chunks, prompt_tokens, completion_tokens
